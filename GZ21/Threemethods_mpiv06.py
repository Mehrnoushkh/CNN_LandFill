...
OUTPUT_DIR = './output_40seeds_4methods'
PREPROCESSED_FILE = './preprocessed_data4x_4methods.npz'
...
#]
#SEEDS = [
   # 101, 207, 314, 428, 533, 641, 759, 862, 973, 1087,
   # 1199, 1312, 1423, 1538, 1647, 1753, 1869, 1971, 2083, 2194,
   # 2307, 2418, 2529, 2637, 2748, 2859, 2967, 3078, 3189, 3297,
   # 3408, 3519, 3627, 3738, 3849, 3957, 4068, 4179, 4287, 4398,
   # 4509, 4617, 4728, 4839, 4947, 5058, 5169, 5277, 5388, 5499,
SEEDS = [ 5607, 5718, 5829, 5937, 6048, 6159, 6267, 6378, 6489, 6597,
    ...
    #10008, 10119, 10227, 10338, 10449, 10557, 10668, 10779, 10887, 10998,]
# Each rank uses all available CPUs (1 rank per node)
...
def compute_subgrid_forcing(T_highres, factor):
    ...
    for t in range(n_times):
        ...
        T_coarse[t] = np.mean(T_t, axis=(1, 3))
        #S_T[t] = np.nanmean(T_t**2, axis=(1, 3)) - T_coarse[t]**2
        S_T[t] = np.mean(T_t**2, axis=(1, 3)) - T_coarse[t]**2
    return T_coarse, S_T
...
def broadcast_array(arr, root=0):
    ...
    return arr
def create_mean_fill(T_with_nan, land_mask_3d):
    """Fill land with global ocean mean temperature."""
    T_mean_fill = T_with_nan.copy()
    ocean_vals = T_with_nan[~land_mask_3d]
    global_mean = np.nanmean(ocean_vals)
    print(f"  Global ocean mean: {global_mean:.2f} K", flush=True)
    T_mean_fill[land_mask_3d] = global_mean
    T_mean_fill = np.nan_to_num(T_mean_fill, nan=global_mean)
    return T_mean_fill
# =============================================================================
# Training functions
# =============================================================================
...
def main():
    ...
    if rank == 0:
        if os.path.exists(PREPROCESSED_FILE):
            ...
            T_coarse_mean = data['T_coarse_mean']
            ...
            T_mean_raw = create_mean_fill(T_nan, land)
            ...
            T_coarse_mean, _ = compute_subgrid_forcing(T_mean_raw, COARSEN_FACTOR)

            log("Saving cache...")
            np.savez(PREPROCESSED_FILE,
                     T_coarse_zero=T_zero, T_coarse_replicate=T_rep,
                     T_coarse_laplace=T_lap, T_coarse_mean=T_coarse_mean,
                     S_T_target_masked=S_T,
                     ocean_mask=ocean_mask, coastal_mask=coastal_mask)

        log(f"Data shape: {T_zero.shape}")
    else:
        T_zero = T_rep = T_lap = T_coarse_mean = S_T = ocean_mask = coastal_mask = None
    ...
    T_coarse_mean = broadcast_array(T_coarse_mean)
    ...
    for my_seed in my_seeds:
        ...
        for name, T in [('zero_fill', T_zero), ('replicate_fill', T_rep),
                 ('mean_fill', T_coarse_mean),('laplace_fill', T_lap)]:
        ...
        np.savez(os.path.join(OUTPUT_DIR, f'spatial_pred_seed_{my_seed}.npz'),
                 ...
                 pred_mean=spatial_preds['mean_fill'],
    ...
    if rank == 0:
        ...
        stats = {m: {'overall': [], 'coastal': [], 'open_ocean': []}
         for m in ['zero_fill', 'replicate_fill', 'mean_fill', 'laplace_fill']}
        ...
        methods = ['zero_fill', 'replicate_fill', 'mean_fill', 'laplace_fill']
        method_labels = ['Zero-fill', 'Replicate-fill', 'Mean-fill', 'Laplace-fill']
        ...
        for seed in SEEDS:
            ...
            all_preds['mean_fill'].append(data['pred_mean'])
        ...
        # Plot 5: R² Bar Chart
        # =====================================================================
        methods = ['Zero-fill', 'Replicate-fill',  'mean_fill','Laplace-fill']
        keys = ['zero_fill', 'replicate_fill', 'mean_fill','laplace_fill']
