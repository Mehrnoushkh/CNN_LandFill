...
with nc.Dataset('/scratch/10081/mkharghani/SST/updated_data/res005/data2023_01deg.nc') as ds:
    T_org = np.array(ds['analysed_sst'][0, :, :])  # force to regular numpy array
    lat_org = np.array(ds['latitude'][:])
    lon_org = np.array(ds['longitude'][:])

land_mask_org = np.isnan(T_org) | (T_org == -999.0)
print(land_mask_org.shape)
print("Land fraction:", land_mask_org.mean())  # should be ~0.3, not 0.0 or 1.0
# =============================================================================
...
PREPROCESSED_FILE = './../preprocessed_data4x_4methods.npz'   #preprocessed_data4X_new.npz'
...
method_labels = ['Zero-fill', 'Replicate-fill','mean_fill', 'Laplace-fill']
...
titles = ['True $S_T$ (Target)',
          'Predicted $S_T$ (Replicate-fill)', 'Predicted $S_T$ (Mean-fill)','Predicted $S_T$ (Laplace-fill)']
data_list = [target_avg,
             avg_preds['replicate_fill'], avg_preds['mean_fill'],avg_preds['laplace_fill']]
...
for region_name, region in regions.items():
    ...
    for ax, data, label in zip(axes[1], zoom_errors, method_labels):
        ...
        im2 = ax.pcolormesh(lon_z, lat_z, masked,
                            cmap=cmap_err, vmin=-err_max_z, vmax=err_max_z, shading='nearest')
        ax.contour(lon_hr2d, lat_hr2d, land_zoom_hr.astype(float),
                   levels=[0.5], colors='green', linewidths=0.5)
        #ax.set_title(f'Error: {label}', fontsize=11)
        #ax.set_xlabel('Longitude (°)')
        #ax.set_ylabel('Latitude (°)')
  ...
  #  cbar_ax2 = fig.add_axes([0.90, 0.10, 0.015, 0.35])
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, f'{region["filename"]}_new.png'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(OUTPUT_DIR, f'{region["filename"]}_new.pdf'), bbox_inches='tight')
    plt.close()
    print(f"✓ {region['filename']}")
...
# =============================================================================
COLORS = ['#3498DB', '#E67E22','gray', '#2ECC71']
