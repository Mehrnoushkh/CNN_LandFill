"""
OPTIMIZED MPI-Parallel 3-method comparison
- Efficient broadcasting with comm.Bcast (not pickle-based)
- Pre-allocated arrays
- Timing for performance tracking
- Option to save/load preprocessed data

Run with: ibrun python threemethods_mpi_optimized.py
"""
...
OUTPUT_DIR = './output_mpi_optimized'
PREPROCESSED_FILE = './preprocessed_data.npz'  # Cache preprocessed data
USE_CACHE = True  # Set to True to skip preprocessing after first run
...
def broadcast_array(arr, root=0):
    """Efficiently broadcast numpy array using MPI."""
    if rank == root:
        shape = arr.shape
        dtype = arr.dtype
    else:
    ...
    # Pre-allocate on receiving ranks
    if rank != root:
        arr = np.empty(shape, dtype=dtype)
    
    # Efficient broadcast (no pickle)
...
def main():
    ...
    if rank == 0:
        print("=" * 60)
        print("MPI PARALLEL CNN TRAINING (OPTIMIZED)")
        print(f"Total ranks: {size}")
        print(f"Total seeds: {len(ALL_SEEDS)}")
        print(f"Seeds per rank: ~{len(ALL_SEEDS) // size}")
        print("=" * 60)
    ...
    if rank == 0:
        ...
        if USE_CACHE and os.path.exists(PREPROCESSED_FILE):
            print(f"\n[Rank 0] Loading preprocessed data from cache...")
            with Timer("Load cache", rank):
                data = np.load(PREPROCESSED_FILE)
                T_coarse_zero = data['T_coarse_zero']
                T_coarse_replicate = data['T_coarse_replicate']
                T_coarse_laplace = data['T_coarse_laplace']
                S_T_target_masked = data['S_T_target_masked']
                ocean_mask = data['ocean_mask']
                coastal_mask = data['coastal_mask']
        else:
            print(f"\n[Rank 0] Loading and preprocessing data...")
            
            with Timer("Load original data", rank):
            ...
            if USE_CACHE:
                print("[Rank 0] Saving preprocessed data to cache...")
                np.savez(PREPROCESSED_FILE,
                         T_coarse_zero=T_coarse_zero,
                         T_coarse_replicate=T_coarse_replicate,
                         T_coarse_laplace=T_coarse_laplace,
                         S_T_target_masked=S_T_target_masked,
                         ocean_mask=ocean_mask,
                         coastal_mask=coastal_mask)
                print(f"  ✓ Cache saved to {PREPROCESSED_FILE}")
        
        print(f"[Rank 0] Data shapes: T_coarse={T_coarse_zero.shape}, ocean_mask={ocean_mask.shape}")
        print("[Rank 0] Broadcasting to other ranks...")
    ...
    with Timer("Broadcast data", rank) if rank == 0 else Timer("Receive data", rank):
        T_coarse_zero = broadcast_array(T_coarse_zero, root=0)
        T_coarse_replicate = broadcast_array(T_coarse_replicate, root=0)
        T_coarse_laplace = broadcast_array(T_coarse_laplace, root=0)
        S_T_target_masked = broadcast_array(S_T_target_masked, root=0)
        ocean_mask = broadcast_array(ocean_mask, root=0)
        coastal_mask = broadcast_array(coastal_mask, root=0)
    ...
    for i, seed in enumerate(my_seeds):
        ...
        print(f"[Rank {rank}] Training seed {seed} ({i+1}/{len(my_seeds)})...")
        ...
        print(f"[Rank {rank}] Seed {seed} done in {seed_time:.1f}s: "
    ...
    # =========================================================================
    print(f"[Rank {rank}] Finished all seeds, gathering results...")
    ...
    if rank == 0:
        ...
        plt.savefig(os.path.join(OUTPUT_DIR, 'r2_comparison_mpi.pdf'), bbox_inches='tight')
        plt.close()
