...
def main():
    ...
    # =========================================================================
    # Load ORIGINAL data (to get ocean mask)
    # =========================================================================
    print(f"\n{'='*60}")
    print("LOADING ORIGINAL DATA (for ocean mask)")
    print(f"{'='*60}")
    ...
    # Convert original to NaN over land (for computing "true" S_T over ocean only)
    ...
    # For FILLED data - use physics-filled values everywhere
    T_coarse_filled, S_T_filled = compute_subgrid_forcing(T_filled, COARSEN_FACTOR)
    
    # For ocean mask - coarsen the original (with NaN) to get ocean points
    T_coarse_orig, S_T_orig = compute_subgrid_forcing(T_orig_with_nan, COARSEN_FACTOR)
    ...
    # Use S_T computed from ORIGINAL data (over ocean) as target
    # This is the "true" subgrid variance without fill artifacts
    S_T_target = S_T_orig.copy()
    S_T_target[:, ~ocean_mask] = 0.0  # Zero out land for loss masking

    print(f"\nS_T (filled) range: [{np.nanmin(S_T_filled):.2f}, {np.nanmax(S_T_filled):.2f}]")
    print(f"S_T (target/ocean) range: [{np.nanmin(S_T_target[S_T_target>0]):.2f}, {np.nanmax(S_T_target):.2f}]")
