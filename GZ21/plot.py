...
Load your trained model and compare predictions vs truth.
...
MODEL_FILE = 'output_single/model_300days.pth'
...
def main():
    ...
    T_raw = ds[T_VAR_NAME].values
    
    # Handle fill values
    T_with_nan = T_raw.copy()
    T_with_nan[T_raw == FILL_VALUE] = np.nan
    T_zero = np.nan_to_num(T_with_nan, nan=0.0)
    
    # Compute forcing
    print("Computing coarse data and forcing...")
    T_coarse, S_T = compute_subgrid_forcing(T_zero, COARSEN_FACTOR)
    T_coarse_orig, _ = compute_subgrid_forcing(T_with_nan, COARSEN_FACTOR)
    ...
    # Pick a few sample days to visualize
    sample_days = [0, 100, 200, 300]  # Day 1, 101, 201, 301
    sample_days = [d for d in sample_days if d < T_coarse.shape[0]]
    ...
    with torch.no_grad():
        for day in sample_days:
            # Prepare input
            T_input = (T_coarse[day] - T_mean) / T_std
            T_input = torch.tensor(T_input, dtype=torch.float32)
            T_input = T_input.unsqueeze(0).unsqueeze(0).to(device)  # (1, 1, H, W)
            ...
            pred_std = output[0, 1].cpu().numpy() * S_std  # std is always positive
    ...
    # Ground truth
    truths = [S_T[day] for day in sample_days]
    ...
    for i, day in enumerate(sample_days):
        ...
        # Find common scale for truth and prediction
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'predictions_maps.png'), dpi=150)
    plt.close()
    print(f"  ✓ Saved predictions_maps.png")
